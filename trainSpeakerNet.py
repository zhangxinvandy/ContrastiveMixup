...
parser.add_argument('--alpha',          type=float, default=0.,     help='mixup interpolation coefficient (default: 0 -> no mixup)')
parser.add_argument('--mixup_mode',   type=str,   default="",  help='overlap|cutmix|melspec')
# For test only
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # when there is no initial model or pretrained model loaded, then the model will be trained from scratch.
    ...
    if args.eval == True:
        ...
        if args.gpu == 0:
            ...
            #writer.add_scalar('Eval/VEER', float(result[1]))
            #writer.add_scalar('Eval/MinDCF', float(mindcf))
            #writer.add_scalar('Eval/Iteration', ii)
    ...
    for it in range(it,args.max_epoch+1):
        ...
        loss, traineer = trainer.train_network(train_loader, verbose=(args.gpu == 0), epoch = it)
